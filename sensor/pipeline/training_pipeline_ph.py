from sensor.entity.config_entity import TrainingPipelineConfig,DataIngestionConfig,DataValidationConfig,DataTransformationConfig,s3DwonloadConfig
...
from sensor.constant.training_pipeline import SAVED_MODEL_DIR,MODEL_FILE_NAME,DATA_TRANSFORMATION_DIR_NAME
from sensor.constant.training_pipeline import DATA_TRANSFORMATION_TRANSFORMED_OBJECT_DIR,PREPROCSSING_OBJECT_FILE_NAME,TARGET_COLUMN
...
class TrainPipeline:
    ...
    def sync_saved_model_dir_to_s3(self, folder):
        try:
            self.folder = folder
            #model_pusher_config = ModelPusherConfig(training_pipeline_config=self.training_pipeline_config)
            aws_buket_url = f"s3://{TRAINING_BUCKET_NAME}/{SAVED_MODEL_DIR}"
            self.s3_sync.sync_folder_to_s3(folder = self.folder,aws_buket_url=aws_buket_url)
    ...
    def run_pipeline(self):
        try:
            ...
            self.sync_saved_model_dir_to_s3(model_pusher_artifact.saved_model_path)
            self.sync_saved_model_dir_to_s3(os.path.dirname(data_transformation_artifact.transformed_object_file_path))
            #print(model_pusher_artifact.saved_model_path)
